...
from .models import Student, Subject, Question, Course, Professor, Result, Done,dept,Comment
...
def credits(request):
    return render(request,'feedback/credits.html')
def analytics(request,br):
    if not request.user.is_authenticated():
        return render(request,'feedback/denied.html')
    students=Student.objects.filter(branch__startswith=br)
    Total_Done=Done.objects.filter(student=students).count()
    Actual_Done=Done.objects.filter(student=students,done=1).count()
    faculty=Professor.objects.filter(dept__startswith=br).order_by('prof_name')
    course=Course.objects.filter(professor=faculty)
    comments=Comment.objects.filter(course=course)
    question=Question.objects.get(pk=21)
    result=Result.objects.filter(course=course).order_by('course')
    total=Result.objects.filter(course=course,question=question).count()
    result_count=Result.objects.filter(course=course,question=question,total_votes__gt=0).count()
    context={'faculty':faculty,'course':course,'result':result,'question':question,'comments':comments,'result_count':result_count,'total':total,'Total_Done':Total_Done,'Actual_Done':Actual_Done}
...
def depart(request):
    ...
    done=Done.objects.filter(done=1).count()
    total=Done.objects.all().count()
    question=Question.objects.get(pk=21)
    student=Student.objects.all().count()
    result_count=Result.objects.filter(course=COURSE,question=question,total_votes__gt=0).count()
    return render(request,'feedback/all_department.html',{'dept':department,'subject':subject,'course':course,'done':done,'total':total,'student':student,'result_count':result_count})
def home(request):
...
def logout_user(request):
    ...
    return render(request, 'feedback/logged_out.html')
...
def polls(request, prof_pk, stud_roll, sub_pk, key):
    ...
    if not key:
        ...
        messages.error(request,"Oops! You have already submitted this feedback")
        url='/feedback/'+student.key+'/'
        return HttpResponseRedirect(url)
...
def getmail(request):
    ...
    if not email in [user.username for user in User.objects.all()]:
        ...
        messages.success(request,'An email has been sent to your email address.Please Follow the link in the email')
